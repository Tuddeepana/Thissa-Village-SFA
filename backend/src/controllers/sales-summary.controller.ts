...
import { getSalesSummary, getVolumeWiseSummary } from '../services/sales-summary.service';
...
export const getVolumeWiseSummaryController = async (req: Request, res: Response) => {
  const q: SalesSummaryQuery = {
    fromDate: typeof req.query.fromDate === 'string' ? req.query.fromDate : undefined,
    toDate: typeof req.query.toDate === 'string' ? req.query.toDate : undefined,
    today: req.query.today === 'true',
  ...
  const data = await getVolumeWiseSummary(q);
  res.json({ success: true, ...data });
};
